...
rx.Observable.prototype.throwIfEmpty = function (err) {
  ...
  if (env.isDebugging()) {
    switched = rx.Observable.empty();
  } else {
    const error = err instanceof Error ? err : new Error(err);
    switched = rx.Observable.throw(error);
  }
...
/**
 * Log onNext's output.
 * @param  {Function} predicate A Function that converts the value to another
 * value. This parameter is optional.
 * @return {rx.Observable} An Observable object.
 */
rx.Observable.prototype.logNext = function (predicate) {
  return this.doOnNext((val) => {
    if (Function.isInstance(predicate)) {
      utils.log(predicate(val));
    } else {
      utils.log(val);
    }
  });
 ...
 * @return {rx.Observable} An Observable object.
 */
rx.Observable.prototype.logError = function () {
  return this.doOnError(utils.log);
};
 ...
 * @return {rx.Observable} An Observable object.
 */
rx.Observable.fromObject = function (obj) {
